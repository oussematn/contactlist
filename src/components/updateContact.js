...
class updateContact extends React.Component {
    ...
    editContactEvent = (e) => {
        ...
        if (nameValid && this.state.email !== '' && phoneValid && !contactExists) {
            ...
            let i = 0;
            let edited = false;
            while ((i < contacts.length) && !edited) {
                if (contacts[i].id === id) {
                    edited = true;
                    contacts[i].name = this.customName();
    ...
    render() {
        return (
            <div>
                <div className="alert-success d-none">Contact successfully updated!</div>
                <div className="alert-danger d-none">Contact already exists!</div>
                <div className="card">
                    <div className="card-header">Edit Contact</div>
                    <div className="card-body">
                        <form onSubmit={this.editContactEvent}>
